...
class Advanced_Ads_Placements {
	...
	public static function get_items_for_placement_markup( $slug, $placement ) {
		...
		if ( $placement_item_type && ! array_key_exists( $placement['item'], $items[ $placement_item_type . 's' ]['items'] ) ) {
			...
			$item   = \Advanced_Ads::get_instance()->get_model()->{$method}( [ 'include' => $placement_item_id ] );
			...
			if ( $item[0] instanceof WP_Post ) {
				$items[ $placement_item_type . 's' ]['items'][ $placement['item'] ]['name'] = $item[0]->post_title;
			} elseif ( $item[0] instanceof Advanced_Ads_Group ) {
				$items[ $placement_item_type . 's' ]['items'][ $placement['item'] ]['name'] = $item[0]->name;
			} else {
